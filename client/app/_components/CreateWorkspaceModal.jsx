...
export default function CreateWorkspaceModal({ open, onClose }) {
  ...
  return (
    ...
    >
      {/* Panel */}
      <div
        className="w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl transition-all"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4.5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Set up an organization workspace for your engineering team.</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={onSubmit} className="px-6 py-6 space-y-5" noValidate>
          ...
          <div className="space-y-1.5">
            <label htmlFor="cw-name" className="block text-xs font-bold uppercase tracking-wider text-slate-700">
            ...
            <input
              ...
              autoComplete="off"
              disabled={busy}
              className="w-full rounded-xl border border-slate-200/80 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-2xs outline-none transition-all placeholder:text-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <label htmlFor="cw-focus" className="block text-xs font-bold uppercase tracking-wider text-slate-700">
            ...
            <div className="relative">
              <select
                ...
                disabled={busy}
                className="w-full rounded-xl border border-slate-200/80 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-2xs outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
              >
                <option value="" disabled>Select primary focus area...</option>
                {FOCUS_OPTIONS.map((f) => (
          ...
          <div className="pt-2 flex items-center justify-end gap-3 border-t border-slate-100">
            <button
              ...
              onClick={handleClose}
              disabled={busy}
              className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={busy}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-semibold text-white shadow-2xs transition-all hover:bg-slate-800 disabled:opacity-60"
            >
              {busy ? (
                <>
